/**
 * Area Allocator & Priority Budgeting Engine
...
const HIGH = 'HIGH';
const MEDIUM = 'MEDIUM';
const LOW_FIXED = 'LOW_FIXED';
const LOW_FILLER = 'LOW_FILLER';
...
function isLowFixed(type) {
  return getRule(type).priority === LOW_FIXED;
}

function isFiller(type) {
 ...
 * Guarantees Σ targetArea == totalPlotArea with proper hierarchical scaling.
 */
function allocateRoomAreas(rooms, plotW, plotL, options = {}) {
  ...
  // Calculate proportional target for fixed/service rooms based on plot scale
  // On larger plots, bathrooms grow slightly (from ~32 sq ft up to ~50 sq ft), but never excessively
  ...
  // Remaining area to distribute among HIGH and MEDIUM rooms
  let flexibleRooms = list.filter((r) => r.priority === HIGH || r.priority === MEDIUM);

  // If ALL rooms are fixed/service rooms, scale them to fill the plot
  if (flexibleRooms.length === 0) {
    ...
    // Ensure fixed sum doesn't exceed 40% of plot area if flexible rooms exist
    ...
    // Ensure Living Room / Hall is strictly the largest space when present
    const living = flexibleRooms.find((r) => r.type === 'Living Room' || r.type === 'Hall');
    if (living) {
      flexibleRooms.forEach((r) => {
        if (r !== living && r.targetArea >= living.targetArea) {
          // Adjust weights so living is at least 15% larger than any single bedroom
    ...
    // Ensure Bedrooms are larger than Kitchen
  ...
  // Final exact sum calibration to match totalPlotArea perfectly
  let currentSum = list.reduce((s, r) => s + r.targetArea, 0);
  let diff = round1(totalPlotArea - currentSum);

  if (Math.abs(diff) > 0.05) {
    // Add/subtract diff from largest flexible room (Living > Master Bed > Bed > Kitchen)
  ...
  // Assign area weights and allocated areas
  list.forEach((r) => {
...
/**
 * Validate placed geometry areas against hard architectural caps.
 */
function validateAreaCaps(placedRooms, plotW, plotL) {
  const failures = [];
  const checklist = [];
...
function validateExactRoomList(placedRooms, requestedInstances) {
  const reqCounts = {};
...
function findOverlappingPairs(placedRooms, eps = 0.05) {
  const pairs = [];
...
function validateLayoutAgainstRoomList(placedRooms, requestedInstances, plotW, plotL) {
