/**
 * Multi-Pattern Constraint-Based Architectural Geometry Solver (Frontend ES Module)
...
export const CORNER_OFFSET = 0.5; // ~150 mm in ft
export const DOOR_WIDTH = 3.0;
export const MAIN_ENTRY_WIDTH = 3.5;
 ...
 */
export function createRoomRect(inst, x, y, width, height, floorLevel) {
 ...
 */
export function solveGeometryFromGraph(graph, W, L, floorLevel, alignStaircase = null, options = {}) {
...
export function sliceBandHorizontally(rooms, x, y, totalW, totalH, floorLevel) {
  if (!rooms || rooms.length === 0) return [];
...
export function sliceBandVertically(rooms, x, y, totalW, totalH, floorLevel) {
  if (!rooms || rooms.length === 0) return [];
  if (rooms.length === 1) return [createRoomRect(rooms[0], x, y, totalW, totalH, floorLevel)];
...
export function generateGuillotineLayout(rooms, W, L, floorLevel) {
  const priorityOrder = {
...
export function getExteriorEdges(room, W, L) {
  const edges = [];
...
export function generateWindows(room, W, L) {
  const windows = [];
...
export function findSharedWallSegment(a, b) {
  if (!a || !b) return null;
...
export function createMainEntryDoor(room, W, L) {
...
export function assignDoorsFromGraph(placed, graph, W, L, floorLevel) {
